...
float get_mpu_temperature() {
    ...
    mpu_read_single_reg(0x41, &temp_H_byte);
    mpu_read_single_reg(0x42, &temp_L_byte);
...
_Bool mpu_write_reg(uint8_t reg_to_write, uint8_t data) {
    uint8_t buffer[] = {reg_to_write, data};
    uint16_t count = I2C_Multi_Write(0x68, buffer, 2);
    if (count == 2) {
        return 1;
    } else {
        return 0;
...
_Bool mpu_read_single_reg(uint8_t reg_to_read, uint8_t *data) {
    _Bool resp = 0;
    resp = I2C_Single_Write(0x68, reg_to_read);
    *data = I2C_Single_Read(0x68);

    return resp;
}

uint16_t mpu_read_multi_reg(uint8_t start_address, uint8_t *data, uint8_t len) {
    if (I2C_Single_Write(0x68, start_address)) {
        uint16_t count = 0;
        count = I2C_Multi_Read(0x68, data, len);
        return count;
    } else {
        return 0;
    }
