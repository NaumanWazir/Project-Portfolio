...
unsigned int I2C_Multi_Write(unsigned char slave_address, unsigned char* ptr, unsigned int len) {
    ...
    while (!I2C_Start());
    _Bool resp = I2C_Transmit_Byte(slave_address);
    if (resp == 0) {
        I2C_Stop();
...
_Bool I2C_Single_Write(unsigned char slave_address, unsigned char data) {
    while (!I2C_Start());
    slave_address = (uint8_t) (slave_address << 1);
    _Bool resp = I2C_Transmit_Byte(slave_address);
    if (resp == 0) {
...
_Bool I2C_Detect_Slave(unsigned char slave_address) {
    slave_address = (uint8_t) (slave_address << 1);
    while (!I2C_Start());
    _Bool resp = I2C_Transmit_Byte(slave_address);
    I2C_Stop();
...
unsigned char I2C_Single_Read(unsigned char slave_address) {
    ...
    if (0 == I2C_Transmit_Byte(slave_address)) {
        I2C_Stop();
...
_Bool I2C_Transmit_Byte(unsigned char data) {
